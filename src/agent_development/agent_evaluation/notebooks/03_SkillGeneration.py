...
if _wheels_path:
    ...
    subprocess.check_call(["pip", "install", "-U", "mlflow", "databricks-sdk", "databricks-openai", "dspy", "openai", "--find-links", _wheels_path, "--no-index", "-q"])
else:
    print("Installing from PyPI...")
    subprocess.check_call(["pip", "install", "-U", "mlflow[genai]>=3.5", "databricks-sdk", "databricks-openai", "dspy>=2.6", "openai", "-q"])
...
"""
    ...
    _sw = _SkillWC()
    _sr = _sw.api_client.do(
        "POST", f"/serving-endpoints/{judge_model}/invocations",
        body={"messages": [{"role": "user", "content": skill_generation_prompt}], "max_tokens": 4096, "temperature": 0.3},
    )
    generated_skills_text = _sr["choices"][0]["message"]["content"]
